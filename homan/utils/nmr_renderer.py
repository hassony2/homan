...
colors = {
    ...
    "yellow": [210 / 255.0, 200 / 255.0, 124 / 255.0],
    "white": [1, 1, 1],
    # "gold": [255 / 255, 215 / 255, 0],
    "gold": [240 / 255, 200 / 255, 0],
    "grey": [204 / 255, 204 / 255, 204 / 255],
}
...
class OrthographicRenderer(object):
    def __init__(self, image_size=256, texture_size=1):
        ...
        self.renderer = nr.Renderer(image_size,
                                    camera_mode="look_at",
                                    perspective=False)
...
class PerspectiveRenderer(object):
    def __init__(self, image_size=256, texture_size=1):
        self.image_size = image_size
        self.default_K = torch.cuda.FloatTensor([[[1, 0, 0.5], [0, 1, 0.5],
                                                  [0, 0, 1]]])
        ...
        self.t_size = texture_size
        self.renderer = nr.renderer.Renderer(image_size=image_size,
                                             K=self.default_K,
                                             R=self.R,
                                             t=self.t,
                                             orig_size=1)
    ...
    def mesh_with_colors(self,
                         ...
                         rotation=None):
        textures = []
        for faces, color_name in zip(faces_list, color_names):
            texture = torch.ones(
                len(faces),
                faces.shape[1],
                self.t_size,
        ...
        for vertices in verts_list:
            trans_verts.append(
                self.transform_points(vertices,
                                      translation=translation,
                                      rotation=rotation))
        vertices = torch.cat(trans_verts, 1)
        faces = torch.cat(faces_list, 1)
        textures = torch.cat(textures, 1)
        rend, depth, sil = self.renderer.render(vertices, faces, textures)
        rend = rend.detach().cpu().numpy().transpose(0, 2, 3,
                                                     1)  # B x H x W x C
    ...
    def __call__(
        ...
        vertices = self.transform_points(vertices,
                                         translation=translation,
                                         rotation=rotation)
    ...
    def transform_points(self, vertices, translation=None, rotation=None):
